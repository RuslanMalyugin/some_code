...
main (int argc, char *argv[])
{
  ...
  if (_file_size / struct_size == 0)
    return 0;
  struct Item current[_file_size / struct_size];
  size_t size_struct = sizeof (current[0]);
  ReadFile (fileHandle, current, sizeof (current), &bytes_read, NULL);
  size_t ptr = 0;
  do
    {
      printf ("%d", current[ptr].value);
      ptr = current[ptr].next_pointer;
    }
  while (ptr != 0);
